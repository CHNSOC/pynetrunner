...
class Identity(Card):
    def __init__(self, card_data):
        super().__init__(card_data)
        self.minimum_deck_size = card_data['attributes'].get(
            'minimum_deck_size', 45)
        self.influence_limit = card_data['attributes'].get(
            'influence_limit', 15)
...
class Program(Card):
    ...
    def can_break(self, ice):
        for ability in self.effects.get('paid_abilities', []):
            if ability['type'] == 'break_subroutine' and (ability['target'] == 'any' or ice.subtype in ability['target']):
                return True
        return False

    def break_subroutine(self, subroutine, player):
        for ability in self.effects.get('paid_abilities', []):
            if ability['type'] == 'break_subroutine' and player.credits >= ability['cost']:
    ...
    def increase_strength(self, amount, player):
        for ability in self.effects.get('paid_abilities', []):
            if ability['type'] == 'increase_strength' and player.credits >= ability['cost']:
                player.credits -= ability['cost']
